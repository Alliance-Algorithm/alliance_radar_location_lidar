...
1. **推理**:`validate_3layer`(L1 检测 → L2 装甲板分类 → L3 号码验证)对 100 张抽帧图(5472×3648)输出检测框中心像素 u,v,共 188 个检测。类别颜色以引擎顺序为准(TensorRT 引擎 `best_fixed_names_1280` 为 red 0-5、blue 6-11,`validate_3layer.cpp` 的 `CLASS_NAMES` 与之一致)。
...
|---|---|---|---|
| frame_057 | eng_b | 3637,3096 | (4.43,-1.70) |
| frame_057 | inf4_b | 3637,3096 | (4.43,-1.70) |
| frame_057 | inf4_b | 3613,3143 | (4.34,-1.79) |
| frame_061 | inf4_b | 4384,2639 | (6.79,-0.76) |
| frame_063 | inf4_b | 2028,1978 | (-10.73,-0.12) |
| frame_065 | inf4_r | 5410,2502 | (8.61,-0.45) |
| frame_071 | inf4_r | 5378,2547 | (8.57,-0.50) |
| frame_075 | inf4_b | 3254,3557 | (3.48,-2.61) |
| frame_081 | inf3_r | 2826,1760 | (1.28,0.40) |
...
|---|---|---|---|---|
| frame_073 | inf3_b | 1574,2024 | (-14.10,-0.33,1.41) | 远端外墙 |
| frame_075 | inf4_r | 1574,2023 | (-14.10,-0.33,1.41) | 远端外墙 |
| frame_061 | eng_b | 1308,1807 | (-0.15,0.32,3.23) | 中心结构顶板 |
| frame_089 | hero_b | 1289,1832 | (-0.20,0.27,3.26) | 中心结构顶板 |

### delta 最大 10 行(双命中)

| # | frame | class | 旧命中 (x,y,z) | 新命中 (x,y,z) | delta |
|---|---|---|---|---|---|
| 1 | frame_081 | hero_r | (-9.38,0.96,0.00) | (0.23,0.60,1.64) | 9.62 m |
| 2 | frame_083 | inf4_b | (-8.86,-0.13,0.00) | (0.45,-0.04,1.62) | 9.31 m |
| 3 | frame_059 | inf3_r | (-6.23,0.65,0.00) | (0.67,0.45,1.36) | 6.90 m |
| 4 | frame_059 | eng_r | (-6.23,0.65,0.00) | (0.67,0.45,1.36) | 6.90 m(同像素重复检测) |
| 5 | frame_071 | inf4_b | (-2.70,-0.50,0.30) | (0.22,-0.40,0.95) | 2.93 m |
| 6 | frame_051 | inf4_b | (-0.13,-0.39,0.60) | (0.36,-0.38,0.72) | 0.49 m |
| 7 | frame_085 | eng_r | (0.21,0.12,3.01) | (-0.21,0.12,2.98) | 0.42 m |
| 8 | frame_069 | hero_b | (0.18,-0.32,0.60) | (0.44,-0.31,0.66) | 0.25 m |
| 9 | frame_093 | inf3_b | (1.07,0.40,0.42) | (1.24,0.40,0.47) | 0.17 m |
| 10 | frame_059 | inf3_r | (1.10,0.42,0.40) | (1.26,0.42,0.45) | 0.16 m |
...
- 后果:新 mesh 把"穿过中心结构的射线"正确挡在结构表面(z 0.95~3.26),旧 mesh 让射线穿过去命中 3~9.6m 外的地面——该差异只发生在 delta >0.5m 的 5 行(旧命中 z 0.00~0.30,其中 4/5 行为 0.00)与 2 个 new_only 顶板命中;delta ≤0.5m 的行(如第 7 行 frame_085,旧 z 3.01 与新 z 2.98 均为顶板)两 mesh 都命中结构表面,仅是高度差,不属遮挡差异。**两面性**:
  - 若中心结构真是实心,新 mesh 的遮挡是对的;但旧 mesh 结果说明这些像素上神经网络"看到了"结构后方的机器人(如 hero_r 位于 (-9.38,0.96))——要么这些是误检,要么新 mesh 重建把结构外扩了 ~0.1-0.3m(Ball Pivoting 膨胀)导致边缘像素被误挡,真实相机可从结构边缘看到机器人。
