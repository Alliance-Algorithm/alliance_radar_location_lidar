...
| 类别 | 风格 | 示例 |
|---|---|---|
| 命名空间 | `snake_case`，包名即命名空间 | `radar_camera` / `radar_bridge` / `radar_fusion` |
...
- 顶层命名空间等于 ROS 包名：`radar_camera` / `radar_lidar` / `radar_fusion`。
- 所有公开可见的命名符号（class / struct / enum / 类型别名 / 编译期常量 / 自由函数）
  **必须**放在有实际含义的具体模块子命名空间中，禁止放在包根命名空间。

  ✅ 正确：
  ```cpp
  namespace radar_camera::model_inference { class ModelInference; }
  namespace radar_camera::projection       { class Projector; }
  namespace radar_camera::node             { class RadarCameraNode; }

  namespace radar_lidar::map_data          { class MapData; }
  namespace radar_lidar::localization      { class LocalizationStage; }
  namespace radar_lidar::spherical_grid    { class SphericalGrid; }
  namespace radar_lidar::frame_accumulator { class FrameAccumulator; }
  namespace radar_lidar::dynamic_cloud     { class DynamicCloudStage; }
  namespace radar_lidar::cluster           { class ClusterStage; class ClusterResult; }
  namespace radar_lidar::node              { class RadarLidarNode; }

  namespace radar_fusion::kalman_tracker   { class KalmanTracker; class KalmanState; enum class TrackLifecycle; }
  namespace radar_fusion::fusion_config    { class FusionConfig; enum class FusionMode; }
  namespace radar_fusion::camera_observation { class CameraObservation; }
  namespace radar_fusion::node             { class RadarFusionNode; }
  ```

  ❌ 禁止（包根下的公开符号）：
  ```cpp
  namespace radar_camera { class CameraNode; }           // 应放入 ::node
  namespace radar_camera { enum class CameraMode; }      // 应放入具体模块
  namespace radar_lidar { void process_scan(); }         // 自由函数不应在根命名空间
  namespace radar_fusion { constexpr int kMaxTracks = 100; }  // 不应在根命名空间
  ```

- 模块子命名空间应反映职责或模块边界，拒绝宽泛的「工具」桶：
  ```cpp
  namespace radar_lidar::utils {    // ❌ 太宽泛，不表达任何语义
    class PointCloudHelper;
    class MathUtils;
  }
  ```

- 紧密关联的支持类型应与使用者同属一个模块子命名空间，无需另开目录级别的命名空间。

- 文件名、可执行目标名、CMake 目标名、ROS 资源名不会因 C++ 命名空间自动变更，
  它们各自遵循自身的命名约定，与 C++ 命名空间层级无关。

- 禁止 `Radar::`（大写）、全局裸类、或与包名不一致的命名空间。
