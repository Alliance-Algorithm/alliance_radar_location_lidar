...
### 阶段三：bringup 编排（设计中，未动代码）

实现架构文档已定义的分层 launch/config 编排（"python 启动"）：

- [ ] T3.1 分层 config：`config/common/localization.yaml`（frame/QoS/map_path/滤波）、
      `config/lidar/odin.yaml` + `config/lidar/mid70.yaml`、`config/system/phase{1,2,3}.yaml`
- [ ] T3.2 `localization.launch.py`：`ComposableNodeContainer` 承载 radar_lidar + radar_fusion，
      `sensor:=odin|mid70` 参数选层，intra-process 打开；`IncludeLaunchDescription` 拉起独立驱动进程
- [ ] T3.3 `localization_gui.launch.py`：phase1 + radar_bridge + Foxglove
- [ ] T3.4 `full_system.launch.py`：+ radar_camera
