...
// x100 with no map_to_rm_offset; negative mode-cell values (padded-field clip allows
...
inline void fill_empty_slots(radar_interfaces::msg::LidarLocation& msg,
    ...
    for (std::size_t i = 0; i < slots.size(); ++i) {
        ...
        msg.*(slots[i].x) = static_cast<std::uint16_t>(std::max(0.0, p.x_mode) * 100.0);
        msg.*(slots[i].y) = static_cast<std::uint16_t>(std::max(0.0, p.y_mode) * 100.0);
    }
}
