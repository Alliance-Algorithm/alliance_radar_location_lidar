...
// stage_remain_time < 10. A message that ends a round never starts the next
// one (reset takes precedence), so the next round re-enters the time series
// from t = 0 on a later start condition.
class MatchTimer {
public:
    ...
    void on_game_state(std::uint8_t game_progress, std::uint16_t stage_remain_time,
        std::int64_t now_ns);
...
inline void MatchTimer::on_game_state(
    ...
    const bool round_ended = game_progress == 5 || stage_remain_time < 10;
    if (round_ended) {
        running_  = false;
        start_ns_ = 0;
        return;
