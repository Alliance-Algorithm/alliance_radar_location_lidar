    ...
    video_thread_         = std::thread([this]() {
        ...
        while (video_thread_running_) {
            ...
            if (!frame_result.has_value()) {
                if (frame_result.error().code == hikcamera::FrameReadErrorCode::Timeout) {
                    continue; // normal: no new frame, bounded by running_ flag
                }
                std::cerr << "[VideoBridge] reader error: " << frame_result.error().message << "\n";
                continue;
            }
            ...
            if (!cv::imencode(".jpg", frame.mat(), jpeg, { cv::IMWRITE_JPEG_QUALITY, 85 })
                || jpeg.empty()) {
