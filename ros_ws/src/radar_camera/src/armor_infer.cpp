...
const char* kNames[] = { "hero-R", "eng-R", "inf3-R", "inf4-R", "sentry-R", "drone-R", "hero-B",
    "eng-B", "inf3-B", "inf4-B", "sentry-B", "drone-B" };
...
auto decode_l2(const std::vector<float>& raw, const cv::Rect2f& roi, float scale, int px, int py,
    ...
    for (size_t i = 0; i + 21 < raw.size(); i += 22) {
        ...
        int color = static_cast<int>(
            std::max_element(raw.begin() + i + 9, raw.begin() + i + 13) - (raw.begin() + i + 9));
        int genre = static_cast<int>(
            std::max_element(raw.begin() + i + 13, raw.begin() + i + 22) - (raw.begin() + i + 13));
        std::vector<cv::Point2f> corners;
        for (int p = 0; p < 4; ++p) {
            corners.emplace_back(
                (raw[i + p * 2] - px) / scale + roi.x, (raw[i + p * 2 + 1] - py) / scale + roi.y);
        }
    ...
    for (size_t i = 0; i < candidates.size(); ++i) {
        ...
        for (size_t j = 0; j < i; ++j)
            if (iou(candidates[i].box, candidates[j].box) > l2_nms) suppressed = true;
...
auto l1_names(int id) -> const char* { return (id >= 0 && id < 12) ? kNames[id] : "?"; }
...
namespace {
    ...
    auto run_l3(model_inference::TensorRtInference& engine, const cv::Mat& frame, cv::Rect2f box,
        float l3_conf) -> std::optional<Number> {
        float scale;
        int px;
        int py;
        ...
        const int index =
            static_cast<int>(std::max_element(raw.begin(), raw.begin() + 9) - raw.begin());
        const float confidence = raw[index];
        if (confidence < l3_conf) return std::nullopt;
        return Number { index, confidence };
    }

    auto run_l2(model_inference::TensorRtInference& engine, const cv::Mat& frame, cv::Rect2f roi,
        float l2_conf) -> std::optional<Plate> {
        float scale;
        int px;
        int py;
    ...
    auto infer_l1(model_inference::TensorRtInference& engine, const cv::Mat& rgb)
        -> std::vector<Det> {
        float scale;
        int px;
        int py;
...
ArmorInfer::ArmorInfer(std::unique_ptr<ArmorInferImpl> impl)
    : impl_(std::move(impl)) { }

auto ArmorInfer::create(const std::string& model_dir, float l1_conf, float l2_conf, float l3_conf)
...
auto ArmorInfer::infer(const cv::Mat& frame_rgb) -> std::vector<ArmorResult> {
    ...
    for (const auto& detection : infer_l1(impl_->l1_engine, frame_rgb)) {
        ...
        result.match_state =
            (l2_match || l3_match || detection.id == 5 || detection.id == 11) ? "MATCH" : "MISS";
