...
auto to_blob(const cv::Mat& image, std::vector<float>& buffer) -> void {
    cv::Mat blob = cv::dnn::blobFromImage(image, 1.0 / 255.0, cv::Size(), cv::Scalar(), false,
        false);
...
auto ArmorRefiner::refine(const cv::Mat& frame, detection::Detection& det,
    ...
    const bool is_drone = std::find(drone_class_ids.begin(), drone_class_ids.end(),
                              static_cast<std::int64_t>(det.id))
        != drone_class_ids.end();
...
auto ArmorRefiner::run_l2(const cv::Mat& frame, const cv::Rect2f& roi)
    ...
    for (int i = 0; i < num; ++i) {
        ...
        const float bx = (min_x - static_cast<float>(pad_x)) / scale + static_cast<float>(crop_rect.x);
        const float by = (min_y - static_cast<float>(pad_y)) / scale + static_cast<float>(crop_rect.y);
...
auto ArmorRefiner::run_l3(const cv::Mat& frame, const cv::Rect2f& plate)
    ...
    for (int i = 1; i < kL3Classes; ++i) max_logit = std::max(max_logit, logits[i]);
    float sum = 0.0f;
    for (int i = 0; i < kL3Classes; ++i) sum += std::exp(logits[i] - max_logit);
...
auto ArmorRefiner::l2_to_class_id(int genre, ArmorColor color) -> std::optional<int> {
    ...
    int blue_id = -1;
    switch (genre) {
        case 1: blue_id = 0; break; // hero
        case 2: blue_id = 1; break; // engineer
        case 3: blue_id = 2; break; // infantry3
        case 4: blue_id = 3; break; // infantry4
        case 6: blue_id = 4; break; // sentry
...
auto ArmorRefiner::l3_idx_to_class_id(int idx) -> std::optional<int> {
    ...
    switch (idx) {
        case 0: return 0;  // B1 blue hero
        case 1: return 1;  // B2 blue engineer
        case 2: return 2;  // B3 blue infantry3
        case 3: return 3;  // B4 blue infantry4
        case 4: return 4;  // BS blue sentry
        case 5: return 6;  // R1 red hero
        case 6: return 7;  // R2 red engineer
        case 7: return 8;  // R3 red infantry3
        case 8: return 9;  // R4 red infantry4
        default: return std::nullopt;
    }
}
