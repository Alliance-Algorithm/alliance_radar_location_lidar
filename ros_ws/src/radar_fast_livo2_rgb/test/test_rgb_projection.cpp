...
TEST(RgbProjection, ProjectsForwardPointIntoCalibratedPixel) {
    const Calibration calibration { .fx = 100.0,
        .fy                             = 100.0,
        .cx                             = 50.0,
        .cy                             = 40.0,
    ...
    const auto projection = project_lidar_point(Eigen::Vector3d { 1.0, 0.0, 2.0 }, calibration);
...
TEST(RgbProjection, RejectsPointBehindCamera) {
    ...
    EXPECT_FALSE(project_lidar_point(Eigen::Vector3d { 0.0, 0.0, -1.0 }, calibration).has_value());
}
...
TEST(RgbProjection, KeepsNearestDepthAtSamePixel) {
    cv::Mat depth(8, 8, CV_32FC1, cv::Scalar(std::numeric_limits<float>::infinity()));
...
TEST(RgbProjection, DetectsInvalidZeroFocalLength) {
    ...
    cal.fx          = 0.0;
    EXPECT_FALSE(project_lidar_point(Eigen::Vector3d { 1.0, 0.0, 2.0 }, cal).has_value());
}
...
TEST(RgbProjection, IsVisibleWhenDepthIsClosest) {
    cv::Mat depth(4, 4, CV_32FC1, cv::Scalar(std::numeric_limits<float>::infinity()));
...
TEST(RgbProjection, PacksRawBgrBytes) { EXPECT_EQ(pack_rgb(0xAA, 0xBB, 0xCC), 0xCCBBAAU); }
...
TEST(RgbProjection, QualityScoreIsFiniteForForwardPoint) {
    ...
    double score = quality_score(Eigen::Vector3d { 1.0, 0.0, 2.0 }, *proj, cal, 100, 80);
