...
constexpr std::array<std::string_view, 5> kChunkFields{
    "x", "y", "z", "rgb", "quality"};
constexpr std::array<std::uint64_t, 5> kChunkSizes{4, 4, 4, 4, 8};
constexpr std::array<std::string_view, 5> kChunkTypes{
    "F", "F", "F", "F", "F"};
...
{
    static const SchemaSpec chunk{
        kChunkFields, kChunkSizes, kChunkTypes, kChunkRecordSize};
    static const SchemaSpec final{
        kFinalFields, kFinalSizes, kFinalTypes, kFinalRecordSize};
...
template <typename T>
bool parse_uint64_list(const std::vector<std::string>& tokens,
                       std::vector<T>& values)
{
    values.clear();
...
bool matches_fields(const std::vector<std::string>& actual,
...
bool matches_types(const std::vector<std::string>& actual,
                   std::span<const std::string_view> expected)
{
    if (actual.size() != expected.size()) {
        return false;
...
bool matches_sizes(const std::vector<std::uint64_t>& actual,
                  ...
                  std::string& error)
{
    ...
    while (std::getline(input, line)) {
        ...
        if (key == "DATA") {
            if (!seen_keys.insert(key).second) {
                set_error(error, path_prefix(path, "header parse") +
                                     "duplicate DATA line");
                return false;
            }
            if (tokens.size() != 2U || tokens[1] != "binary") {
                set_error(error, path_prefix(path, "header parse") +
                                     "DATA must be exactly 'DATA binary'");
                return false;
            }
            data_seen = true;
            ...
            if (position == std::streampos(-1)) {
                set_error(error, path_prefix(path, "header parse") +
                                     "cannot determine payload offset");
                return false;
            }
        ...
        if (key == "VERSION") {
            ...
            if (tokens.size() != 8U) {
                set_error(error, path_prefix(path, "header parse") +
                                     "VIEWPOINT must contain seven values");
                return false;
            }
            for (std::size_t i = 1; i < tokens.size(); ++i) {
    ...
    if (!data_seen) {
        set_error(error, path_prefix(path, "header parse") +
                             "missing DATA binary line");
        return false;
    }
    if (fields.empty() || sizes.empty() || types.empty() || counts.empty() ||
        !width.has_value() || !height.has_value() || !points.has_value()) {
        set_error(error, path_prefix(path, "header parse") +
                             "missing required header entry");
        return false;
    }
    if (!matches_fields(fields, spec.fields)) {
        set_error(error, path_prefix(path, "header parse") +
                             "unsupported or incomplete field list");
        return false;
    }
...
bool checked_payload_size(std::size_t count,
                          std::size_t record_size,
...
bool check_payload_length(const fs::path& path,
                          ...
                          std::string& error)
{
    ...
    if (file_error) {
        set_error(error, path_prefix(path, "payload validation") +
                             file_error.message());
        return false;
    }
    const auto expected_file_size = header.data_offset +
                                    static_cast<std::uintmax_t>(payload_size);
...
bool decode_chunk(const std::byte* bytes,
                  RgbChunkRecord& record,
                  std::string& reason)
{
    ...
    if (!std::isfinite(record.x) || !std::isfinite(record.y) ||
        !std::isfinite(record.z) || !std::isfinite(record.quality)) {
...
bool decode_final(const std::byte* bytes,
                  RgbFinalRecord& record,
                  std::string& reason)
{
    std::memcpy(&record.x, bytes, sizeof(record.x));
    std::memcpy(&record.y, bytes + 4, sizeof(record.y));
...
bool read_records(const fs::path& path,
                  const SchemaSpec& spec,
                  ...
                  Decoder&& decoder)
{
    ...
    if (header.count > records.max_size()) {
        set_error(error, path_prefix(path, "payload read") +
                             "POINTS exceeds vector capacity");
        return false;
    }
...
bool records_equal(const RgbChunkRecord& expected,
                   const RgbChunkRecord& actual)
{
    return std::memcmp(&expected.x, &actual.x, sizeof(expected.x)) == 0 &&
           ...
           std::memcmp(&expected.quality, &actual.quality,
                       sizeof(expected.quality)) == 0;
}

bool records_equal(const RgbFinalRecord& expected,
                   const RgbFinalRecord& actual)
{
    return std::memcmp(&expected.x, &actual.x, sizeof(expected.x)) == 0 &&
           std::memcmp(&expected.y, &actual.y, sizeof(expected.y)) == 0 &&
           std::memcmp(&expected.z, &actual.z, sizeof(expected.z)) == 0 &&
...
bool validate_file(const fs::path& path,
                   std::span<const RgbChunkRecord> expected,
                   std::string& error)
{
    std::vector<RgbChunkRecord> actual;
    if (!read_records(path, schema_for(RecordKind::Chunk), actual, error,
                      decode_chunk)) {
        return false;
    }
    if (actual.size() != expected.size()) {
        set_error(error, path_prefix(path, "validate") +
                             "record count does not match input");
        return false;
    }
    for (std::size_t i = 0; i < expected.size(); ++i) {
        if (!records_equal(expected[i], actual[i])) {
            set_error(error, path_prefix(path, "validate") +
                                 "read-back record differs from input");
            return false;
        }
    }
    return true;
...
bool validate_file(const fs::path& path,
                   std::span<const RgbFinalRecord> expected,
                   std::string& error)
{
    std::vector<RgbFinalRecord> actual;
    if (!read_records(path, schema_for(RecordKind::Final), actual, error,
                      decode_final)) {
        return false;
    }
    if (actual.size() != expected.size()) {
...
bool encode_chunk(const RgbChunkRecord& record,
                  std::byte* bytes,
                  std::string& reason)
{
    (void)reason;
...
bool encode_final(const RgbFinalRecord& record,
                  std::byte* bytes,
                  std::string& reason)
{
    (void)reason;
...
bool make_parent_directory(const fs::path& formal, std::string& error)
{
    ...
    if (directory_error) {
        ...
        if (!fs::is_directory(parent, status_error) || status_error) {
            set_error(error, path_prefix(formal, "create parent directory") +
...
bool fsync_file(const fs::path& path, std::string& error)
{
    ...
    if (fd < 0) {
        set_error(error, path_prefix(path, "open temporary file for fsync") +
                             errno_message(errno));
        return false;
    }
    if (::fsync(fd) != 0) {
        ...
        (void)::close(fd);
        set_error(error, path_prefix(path, "fsync temporary file") +
                             errno_message(saved_errno));
        return false;
    }
    if (::close(fd) != 0) {
        set_error(error, path_prefix(path, "close temporary file") +
                             errno_message(errno));
        return false;
    }
    return true;
...
bool fsync_parent_directory(const fs::path& formal, std::string& error)
{
    ...
    if (fd < 0) {
        set_error(error, path_prefix(formal, "open parent directory for fsync") +
                             errno_message(errno));
        return false;
    }
    if (::fsync(fd) != 0) {
        ...
        set_error(error, path_prefix(formal, "fsync parent directory") +
                             errno_message(saved_errno));
        return false;
    }
    if (::close(fd) != 0) {
        set_error(error, path_prefix(formal, "close parent directory") +
                             errno_message(errno));
        return false;
    }
    return true;
...
bool write_transactional(const fs::path& formal,
                          ...
                          std::string& error)
{
    ...
    if (fd < 0) {
        set_error(error, path_prefix(temporary, "create temporary file") +
                             errno_message(errno));
        return false;
    }
    ...
    auto fail = [&](std::string message) {
        if (::close(fd) != 0 && message.empty()) {
            message = path_prefix(temporary, "close temporary file") +
                      errno_message(errno);
        }
    ...
    if (::close(fd) != 0) {
        set_error(error, path_prefix(temporary, "close temporary file") +
                             errno_message(errno));
        remove_owned_temp(temporary, identity);
        return false;
    }
    ...
    if (::rename(temporary.c_str(), formal.c_str()) != 0) {
        const int saved_errno = errno;
        remove_owned_temp(temporary, identity);
        set_error(error, path_prefix(formal, "rename temporary file") +
                             errno_message(saved_errno));
        return false;
    }
...
bool write_rgb_chunk_transactional(const fs::path& path,
                                   std::span<const RgbChunkRecord> records,
                                   std::string& error)
{
    return write_transactional(path, records, schema_for(RecordKind::Chunk),
...
bool write_rgb_final_transactional(const fs::path& path,
                                   std::span<const RgbFinalRecord> records,
                                   std::string& error)
{
    return write_transactional(path, records, schema_for(RecordKind::Final),
...
bool read_rgb_chunk(const fs::path& path,
                    std::vector<RgbChunkRecord>& records,
                    std::string& error)
{
    return read_records(path, schema_for(RecordKind::Chunk), records, error,
                        decode_chunk);
}

bool read_rgb_final(const fs::path& path,
                    std::vector<RgbFinalRecord>& records,
                    std::string& error)
{
    return read_records(path, schema_for(RecordKind::Final), records, error,
                        decode_final);
}

bool validate_rgb_chunk_file(const fs::path& path,
...
bool validate_rgb_final_file(const fs::path& path,
                             std::size_t expected_count,
                             std::string& error)
{
    std::vector<RgbFinalRecord> records;
