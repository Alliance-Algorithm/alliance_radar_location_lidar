...
auto build_camera_info(uint32_t width, uint32_t height, double fx, double fy, double cx, double cy,
    const std::vector<double>& distortion, const std::string& frame_id)
...
public:
    CameraRecorderNode()
        ...
        RCLCPP_INFO(get_logger(),
            ...
            width_, height_, shm_name_.c_str(), image_pub_->get_topic_name(),
...
private:
    ...
    void load_parameters() {
        ...
        if (auto err = validate_image_size(width_, height_, static_cast<size_t>(MAX_IMAGE_SIZE))) {
        ...
        camera_info_msg_ = build_camera_info(static_cast<uint32_t>(width_),
            static_cast<uint32_t>(height_), fx, fy, cx, cy, distortion, camera_frame_id_);
    ...
    void open_shm() {
        ...
        RCLCPP_INFO(get_logger(), "SHM persistent mapping: fd=%d ptr=%p name='%s'", shm_fd_,
            static_cast<void*>(shm_ptr_), shm_name_.c_str());
    }
    ...
    void create_publishers() {
        ...
        auto image_qos = rclcpp::QoS(rclcpp::KeepLast(2)).best_effort().durability_volatile();

        image_pub_ =
            create_publisher<sensor_msgs::msg::Image>("/fast_livo2/camera/bgr8", image_qos);

        // CameraInfo: transient-local reliable — new subscribers
        // receive the latest calibration automatically.
        auto cinfo_qos = rclcpp::QoS(rclcpp::KeepLast(1)).reliable().transient_local();
        ...
        while (running_.load(std::memory_order_acquire)) {
            ...
            for (int retry = 0; retry < kMaxCopyRetries; ++retry) {
                ...
                if (is_frame_stable(counter_before, counter_after)) {
                    uint64_t host_ns =
                        static_cast<uint64_t>(ts_monotonic_copy.time_since_epoch().count());
                    auto ros_stamp = camera_stamp_from_shm(host_ns, img_time_offset_);

                    fill_bgr8_image_metadata(
                        image_msg, width_, height_, ros_stamp, camera_frame_id_);
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        RCLCPP_FATAL(rclcpp::get_logger("camera_recorder_node"), "Startup failed: %s", e.what());
        rclcpp::shutdown();
        return EXIT_FAILURE;
    }
