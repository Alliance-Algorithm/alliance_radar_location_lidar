...
inline auto camera_stamp_from_shm(uint64_t host_monotonic_ns, double img_time_offset)
...
/// bytes in image.data.
inline auto make_bgr8_image(const cv::Mat& bgr, const builtin_interfaces::msg::Time& stamp,
...
inline void fill_bgr8_image_metadata(sensor_msgs::msg::Image& image, int width, int height,
...
inline auto is_finite_positive(double v) -> bool { return std::isfinite(v) && v > 0.0; }

inline auto validate_calibration(double fx, double fy, double cx, double cy,
    const std::vector<double>& distortion) -> std::optional<std::string> {
    if (!is_finite_positive(fx)) return "fx must be finite and positive, got " + std::to_string(fx);
    if (!is_finite_positive(fy)) return "fy must be finite and positive, got " + std::to_string(fy);
    if (!is_finite_positive(cx)) return "cx must be finite and positive, got " + std::to_string(cx);
    if (!is_finite_positive(cy)) return "cy must be finite and positive, got " + std::to_string(cy);
    if (distortion.size() != 5) return "distortion must have exactly 5 coefficients (plumb_bob)";

    bool all_zero = true;
    for (auto d : distortion) {
        if (!std::isfinite(d)) return "distortion coefficient is non-finite";
        if (d != 0.0) all_zero = false;
    }
    if (all_zero) return "distortion coefficients are all zero (placeholder)";
    return std::nullopt;
}
...
inline auto validate_image_size(int width, int height, size_t max_image_bytes)
    -> std::optional<std::string> {
    if (width <= 0) return "width must be positive, got " + std::to_string(width);
    if (height <= 0) return "height must be positive, got " + std::to_string(height);
    ...
    size_t pixels = w * h;
    if (pixels / h != w) return "width * height overflow";

    size_t image_bytes = pixels * kChannels;
    if (image_bytes / kChannels != pixels) return "width * height * 3 overflow";

    if (image_bytes > max_image_bytes)
        return "BGR image size " + std::to_string(image_bytes) + " exceeds max "
            + std::to_string(max_image_bytes);
...
inline auto completed_slot_from_counter(uint64_t frame_counter, unsigned int slot_num)
    -> unsigned int {
    return static_cast<unsigned int>((frame_counter - 1) % static_cast<uint64_t>(slot_num));
}

inline auto is_valid_frame_counter(uint64_t frame_counter) -> bool { return frame_counter > 0; }

inline auto is_frame_stable(uint64_t before, uint64_t after) -> bool { return before == after; }
