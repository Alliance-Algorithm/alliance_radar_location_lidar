...
inline auto validate_calibration(const Calibration& cal) -> bool {
    if (!std::isfinite(cal.fx) || cal.fx <= 0.0) return false;
    if (!std::isfinite(cal.fy) || cal.fy <= 0.0) return false;
    if (!std::isfinite(cal.cx) || cal.cx <= 0.0) return false;
    if (!std::isfinite(cal.cy) || cal.cy <= 0.0) return false;
    if (!cal.rotation_lidar_camera.allFinite()) return false;
    if (!cal.translation_lidar_camera.allFinite()) return false;
    return true;
}
...
inline auto project_lidar_point(const Eigen::Vector3d& point_lidar, const Calibration& cal)
    -> std::optional<Projection> {
    if (!validate_calibration(cal)) return std::nullopt;

    const Eigen::Vector3d p_cam =
        cal.rotation_lidar_camera * point_lidar + cal.translation_lidar_camera;

    if (p_cam.z() <= 0.0) return std::nullopt;
...
inline void update_nearest_depth(cv::Mat& depth_buffer, int col, int row, float depth) {
...
inline auto build_depth_buffer(int width, int height) -> cv::Mat {
    return cv::Mat(height, width, CV_32FC1, cv::Scalar(std::numeric_limits<float>::infinity()));
}

/// Check whether a point at given depth is visible (not occluded) at (col, row).
inline auto is_visible(
    const cv::Mat& depth_buffer, int col, int row, float depth, float tolerance = 1e-6F) -> bool {
    if (col < 0 || col >= depth_buffer.cols || row < 0 || row >= depth_buffer.rows) {
        return false;
    }
...
inline auto pack_rgb(const cv::Vec3b& bgr) -> uint32_t {
    return (static_cast<uint32_t>(bgr[2]) << 16) | (static_cast<uint32_t>(bgr[1]) << 8)
...
inline auto pack_rgb_from_rgb_order(const cv::Vec3b& rgb) -> uint32_t {
    return (static_cast<uint32_t>(rgb[0]) << 16) | (static_cast<uint32_t>(rgb[1]) << 8)
...
inline auto quality_score(const Eigen::Vector3d& point_lidar, const Projection& proj,
    const Calibration& cal, int image_width, int image_height, double normalized_gradient = 0.0)
    -> double {
