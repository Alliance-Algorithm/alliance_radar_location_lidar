...
namespace {
    ...
    auto load_localization_config(rclcpp::Node& node) -> config::LocalizationConfig {
        ...
        node.get_parameter_or(
            "watchdog_check_interval", cfg.watchdog_check_interval, 10);
        node.get_parameter_or("watchdog_unlock_frames", cfg.watchdog_unlock_frames, 3);
        node.get_parameter_or(
            "enable_coarse_relocalize", cfg.enable_coarse_relocalize, false);
        ...
        node.get_parameter_or(
            "coarse_translate_range_m", cfg.coarse_translate_range_m, 6.0);
        node.get_parameter_or(
            "coarse_translate_step_m", cfg.coarse_translate_step_m, 2.0);
