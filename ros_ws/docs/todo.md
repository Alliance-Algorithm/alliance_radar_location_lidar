...
- [x] 新增 `competition.launch.py`：一键启动相机 → LiDAR → fusion → bridge
- [x] 新增 `red_initial_pose.yaml` / `blue_initial_pose.yaml`（红 x=-14 / 蓝 x=+14, z=4, yaw 差 180°）
- [x] `localization.launch.py` 新增 `side` 参数加载对应初始位姿
- [x] 相机升级：MV-CS016-10UC (1.6MP) → **MV-CS200-10UC (20MP, 5472×3648)**

## RM 坐标系变换 /lidar/location（2026-07-20 完成）

- [x] Fusion 新增 RM 坐标系变换：`map_to_rm_offset` 统一处理场地偏移
- [x] Fusion 新增 `/lidar/location` 发布器（24 字段对手/我方机器人坐标）→ radar_bridge
- [x] `LidarLocation.msg` 扩展适配 RM 坐标规范
...
- [x] T3.2 添加相机订阅到 `FusionNode`
  - 订阅 `/radar_camera/robot_pose` 话题（`radar_interfaces::msg::CameraDetectionPose`）
...
- [x] **FAST-LIVO2 移植**：ros2 端口适配 Odin1 全局快门 dToF LiDAR + 高频 IMU，LIO 真实硬件验证通过（漂移率 0.05%）
- [x] **相机内参标定脚本**：`tools/camera_calib/calib_camera.py`，SHM 直读采图 + 离线标定
- [x] **建图后处理工具**：`tools/pcd_postprocess/pcd_postprocess.py`，voxel 降采样 + 离群点剔除
...
- [x] `radar_camera` 包通信框架完成：ConfigsLoader + ImageCallback + PublishCallback，
      CameraDetectionPose 发布到 /radar_camera/robot_pose，FusionNode 消费
